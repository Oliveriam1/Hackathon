...
from .geolocation import CameraModel, DronePose, GimbalAngles
...
class FlightRunner:
    ...
    def camera_step(self, frame, *, received_at, sample_time, source):
        ...
        count = 0
        if (tape.get('state') == 'TAPE_CANDIDATES' and tape.get('live_observation') and vehicle is not None
                and angles is not None and math.isfinite(height) and height >= 1.):
    ...
    def mission_step(self, now=None):
        ...
        if self.fake_camera:
            camera_ready, locked, target = True, False, None
        else:
        ...
        data = self.backend.mission_input(field, camera_ready=camera_ready, camera_locked=locked,
                                          target=target, stop_requested=self.stop_requested)
