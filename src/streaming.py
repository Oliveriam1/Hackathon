...
class VideoSnapshot:
    ...
    telemetry_at: float
    detection: DetectionResult | None  # nebo vision.Observation (kreslí se sama)
    detection_at: float
    camera_status: str


def _draw_detection(frame: np.ndarray, detection: object | None) -> np.ndarray:
    """Observation z Vision se kreslí sama, DetectionResult přes target_detector.annotate."""
    if hasattr(detection, "annotate"):
        return detection.annotate(frame)
    return annotate(frame, detection if detection is not None else DetectionResult((), 0))


def _describe_detection(detection: object) -> str:
    if hasattr(detection, "summary"):
        return detection.summary
    return f"CIRCLES: {len(detection.targets)} | QUADS: {detection.quadrilateral_count}"
...
def draw_overlay(snapshot: VideoSnapshot, *, now: float | None = None) -> np.ndarray:
    ...
    output = _draw_detection(frame, result if fresh_detection else None)
    ...
    if result is None:
        ...
        lines.append(f"{_describe_detection(result)} | age: {detection_age:.2f}s")
