...
Druhá část souboru (z větve test) slouží misi: pevná montáž kamery
(CameraMount), výška AGL a převody GPS <-> metry od bodu startu.
"""
...
# --- Mise (větev test): pevná montáž kamery, AGL, metry od bodu startu. ---
def _radii(origin: GeoCoordinate) -> tuple[float, float]:
    ...
    d = 1 - WGS84_E2 * math.sin(lat) ** 2
    return WGS84_A * (1 - WGS84_E2) / d ** 1.5, WGS84_A / math.sqrt(d) * math.cos(lat)
